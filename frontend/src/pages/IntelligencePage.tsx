...
export const IntelligencePage: React.FC = () => {
  ...
  const decisionPieData = [
    ...
    { name: 'REFRESH', value: decisionCounts.REFRESH, color: '#00F0FF' },
    { name: 'PRE-CACHE', value: decisionCounts.PRE_CACHE, color: '#8B5CF6' },
    { name: 'EVICTIONS', value: decisionCounts.EVICT, color: '#EF4444' },
  ].filter(d => d.value > 0);

  // Object Popularity Histogram (Top 12 accessed objects)
          ...
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={popularityData}>
                ...
                <Bar dataKey="accesses" name="Access Count" fill="#00F0FF" radius={[4, 4, 0, 0]} />
        ...
        <div className="bg-dark-900 border border-dark-750 rounded-xl p-4 shadow-sm">
          ...
          <div className="h-64 w-full flex items-center justify-center">
            {decisionPieData.length > 0 ? (
              ...
              <div className="text-xs text-slate-400 font-mono">No decision data yet</div>
            )}
          </div>
        </div>
      </div>
                ...
                <button
                  ...
                  className={`px-2 py-0.5 rounded transition-colors ${
                    selectedDecisionFilter === filter
                      ? 'bg-dark-700 text-white font-bold'
            ...
            <tbody className="divide-y divide-dark-800">
              {filteredObjects.length > 0 ? (
                filteredObjects.map((obj) => (
                  ...
                  >
                    <td className="py-2.5 font-bold text-white group-hover:text-brand-cyan">{obj.objectId}</td>
                    ...
                    <td className="py-2.5 font-bold text-white">{(obj.adaptiveScore || 0.5).toFixed(2)}</td>
