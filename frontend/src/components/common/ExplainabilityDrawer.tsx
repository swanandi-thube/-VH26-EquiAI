/**
 * Decision Explainability Modal / Drawer
...
export const ExplainabilityDrawer: React.FC = () => {
  ...
  const getDecisionBadge = (type?: string) => {
    switch (type) {
      case 'PRE-CACHE':
        return <span className="bg-brand-purple/20 text-brand-purple border border-brand-purple/40 px-2.5 py-1 rounded font-mono font-bold text-xs">PRE-CACHE</span>;
      case 'REFRESH':
        return <span className="bg-brand-cyan/20 text-brand-cyan border border-brand-cyan/40 px-2.5 py-1 rounded font-mono font-bold text-xs">REFRESH</span>;
      case 'EVICT':
        return <span className="bg-brand-rose/20 text-brand-rose border border-brand-rose/40 px-2.5 py-1 rounded font-mono font-bold text-xs">EVICT</span>;
      case 'KEEP':
      default:
        return <span className="bg-brand-emerald/20 text-brand-emerald border border-brand-emerald/40 px-2.5 py-1 rounded font-mono font-bold text-xs">KEEP</span>;
    }
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="font-bold text-lg text-white">Decision Explainability Inspector</h3>
                {exp && getDecisionBadge(exp.decisionType)}
              </div>
              <p className="text-xs font-mono text-slate-400">
                Target: <span className="text-white font-semibold">{exp?.objectId || 'Loading...'}</span> | Decision ID: <span className="text-slate-300">{selectedDecisionId}</span>
              </p>
            </div>
          </div>
          <button
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="bg-dark-850 border border-dark-750 rounded-xl p-3.5">
                <span className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">Adaptive Score</span>
                <div className="text-2xl font-bold font-mono text-brand-cyan mt-1">
                  {exp.adaptiveScore.toFixed(2)}
                </div>
                <div className="text-[10px] text-slate-400 mt-1">Scale: 0.00 (Evict) to 1.00 (Retain)</div>
              </div>

              <div className="bg-dark-850 border border-dark-750 rounded-xl p-3.5">
                <span className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">Predicted Demand</span>
                ...
                <div className="text-[10px] text-slate-400 mt-1">Confidence: {(exp.confidence * 100).toFixed(0)}%</div>
              </div>

              <div className="bg-dark-850 border border-dark-750 rounded-xl p-3.5">
                <span className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">TTL Calibration</span>
            ...
            <div className="bg-dark-850 border border-dark-700/80 rounded-xl p-4">
              ...
              <p className="text-sm text-slate-200 leading-relaxed font-sans bg-dark-900 p-3 rounded-lg border border-dark-750">
                "{exp.reason}"
              </p>
                    ...
                    <div key={attr.key} className="p-3 bg-dark-850 hover:bg-dark-800 transition-colors flex items-center justify-between">
                      <div className="space-y-0.5">
                        <div className="text-xs font-semibold text-slate-200 flex items-center gap-2">
                          <span>{attr.name}</span>
                          <span className="text-[10px] font-mono text-slate-400">(weight: {attr.weight.toFixed(2)})</span>
                        </div>
                        <p className="text-[11px] text-slate-400">{attr.description}</p>
                      </div>
                      ...
                      <div className="text-right pl-4 shrink-0 font-mono">
                        ...
                        <div className="text-[10px] text-slate-400">Raw: {attr.rawValue.toFixed(2)}</div>
                      </div>
                    </div>
                  );
                })}
            ...
            {/* Timestamp */}
            <div className="text-[11px] text-slate-400 font-mono text-right pt-2 border-t border-dark-800">
