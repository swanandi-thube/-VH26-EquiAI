...
class ObservationRepository {
    ...
    seedFallbackObservations() {
        const now = Date.now();
        for (let cIdx = 0; cIdx < commodityCatalog_1.COMMODITY_CATALOG.length; cIdx++) {
            const item = commodityCatalog_1.COMMODITY_CATALOG[cIdx];
    ...
    async getRecentObservations(objectId, limit = 50) {
        if (client_1.dbClient.isConnected) {
            try {
                const res = await client_1.dbClient.query(`SELECT id, object_id, product_name, category, location, price, previous_price, price_change_pct,
                  timestamp, source, source_reference, data_status, request_count, demand, inventory,
                  backend_latency, retrieval_cost, response_size
