...
class SettingsRepository {
    fallbackSettings = { ...exports.DEFAULT_SETTINGS };
    /**
    ...
    async getSettings() {
        if (client_1.dbClient.isConnected) {
            try {
                const res = await client_1.dbClient.query('SELECT * FROM system_settings WHERE id = $1', ['default']);
    ...
    async updateSettings(patch) {
        ...
        if (client_1.dbClient.isConnected) {
            try {
                ...
                await client_1.dbClient.query(`INSERT INTO system_settings (
