...
import {
  WorkloadUploadSummary,
  WorkloadRequestRecord,
  WorkloadValidationError,
} from '../types';
import { workloadRepository } from '../repositories/workloadRepository';

export interface IngestionResult {
  summary: WorkloadUploadSummary;
  requests: WorkloadRequestRecord[];
}

export class WorkloadIngestionService {
  /**
  ...
  public async ingestFile(
    ...
    const workloadId = `WL-${uuidv4().substring(0, 8)}`;
    const uploadedAt = Date.now();

    if (!content || content.trim().length === 0) {
      const summary: WorkloadUploadSummary = {
        workloadId,
        filename,
      ...
      await workloadRepository.saveWorkloadRun(summary, []);
      return { summary, requests: [] };
    }

    let parsedResult: { requests: WorkloadRequestRecord[]; errors: WorkloadValidationError[]; totalRows: number };
    ...
    for (const req of requests) {
      ...
      if (req.timestamp < minTime) minTime = req.timestamp;
      if (req.timestamp > maxTime) maxTime = req.timestamp;
    }
    ...
    await workloadRepository.saveWorkloadRun(summary, requests);
   ...
   */
  public parseCsvWorkload(content: string): {
    requests: WorkloadRequestRecord[];
    errors: WorkloadValidationError[];
    totalRows: number;
  } {
    const lines = content
      .split(/\r?\n/)
    ...
    const requests: WorkloadRequestRecord[] = [];
   ...
   */
  public parseJsonWorkload(content: string): {
    requests: WorkloadRequestRecord[];
    errors: WorkloadValidationError[];
    totalRows: number;
  } {
  ...
  public validateAndTransformRow(
    ...
    rowNumber: number
  ): { valid: boolean; record?: WorkloadRequestRecord; error?: string } {
    ...
    const record: WorkloadRequestRecord = {
      requestId,
   ...
   */
  private parseNonNegativeNumber(val: any): number | null {
...
export const workloadIngestionService = new WorkloadIngestionService();
