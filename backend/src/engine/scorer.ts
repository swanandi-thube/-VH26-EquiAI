...
import { CacheObjectMetadata, ScoreFactors, SystemSettings } from '../types';
...
export class AdaptiveScorer {
  ...
  public calculateFactors(
    metadata: Partial<CacheObjectMetadata>,
    ...
    if (predictedDemand === undefined || confidence === undefined) {
      const pred = predictor.predictDemand(objectId, now);
...
export const scorer = new AdaptiveScorer();
