...
export class ChangeDetector {
  ...
  public analyze(
    objectId: string,
    currentObs: ObjectObservationRecord,
    history: ObjectObservationRecord[] = []
  ): ChangeDetectionResult {
    ...
    let detectedPattern: DemandTrendType = 'STABLE_DEMAND';
    ...
    let recommendedDecision: DecisionType = 'KEEP';
   ...
   */
  public async analyzeFromRepository(objectId: string, currentObs?: ObjectObservationRecord): Promise<ChangeDetectionResult> {
    const history = await observationRepository.getRecentObservations(objectId, 50);
