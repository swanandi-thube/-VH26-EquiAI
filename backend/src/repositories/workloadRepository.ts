...
import { WorkloadUploadSummary, WorkloadRequestRecord } from '../types';

export class WorkloadRepository {
  private fallbackRuns: Map<string, WorkloadUploadSummary> = new Map();
  private fallbackRequests: Map<string, WorkloadRequestRecord[]> = new Map();

  /**
   * Save a newly ingested workload run and its request trace records
   */
  public async saveWorkloadRun(
        ...
        for (let i = 0; i < requests.length; i += chunkSize) {
          ...
          for (let j = 0; j < chunk.length; j++) {
            ...
            queryParams.push(
              ...
              req.objectId,
              req.operation || 'GET',
              req.responseSizeBytes || 0,
              req.backendLatencyMs || 0,
              req.regenerationCostMs || 0,
              req.statusCode || 200,
              req.ttl ?? null,
   ...
   */
  public async getWorkloadRunById(workloadId: string): Promise<WorkloadUploadSummary | null> {
    if (dbClient.isConnected) {
      try {
        const res = await dbClient.query(
          `SELECT id, filename, file_type, file_size_bytes, total_rows, valid_rows,
   ...
   */
  public async getAllWorkloadRuns(limit = 100): Promise<WorkloadUploadSummary[]> {
    if (dbClient.isConnected) {
      try {
        const res = await dbClient.query(
          `SELECT id, filename, file_type, file_size_bytes, total_rows, valid_rows,
   ...
   */
  public async getWorkloadRequests(
    workloadId: string,
    limit = 1000,
    offset = 0
  ): Promise<WorkloadRequestRecord[]> {
    if (dbClient.isConnected) {
      try {
        const res = await dbClient.query(
          `SELECT request_id, timestamp, object_id, operation, response_size,
   ...
   */
  public async deleteWorkloadRun(workloadId: string): Promise<boolean> {
    this.fallbackRuns.delete(workloadId);
    this.fallbackRequests.delete(workloadId);

    if (dbClient.isConnected) {
      try {
        await dbClient.query('DELETE FROM workload_runs WHERE id = $1', [workloadId]);
        return true;
      } catch (err: any) {
